...
The walkthrough advises to run
...
The only actionable information provided in the scenario that can aid in starting the incestigateion is that there is a suspected Gh0st RAT malware infection.
...
While filtering on events generated only by `onion-fn1` reduced the log count by more than half, it is still too many entries for manual analysis. A technique called stack counting, or long tail analysis, can be used to further reduce the log cound by focusing on a single interesting field. In this case the field called `"alert.signature"` appears to provide a short description of what triggered the IDS alert. This field is a good candidate for stack counting.
