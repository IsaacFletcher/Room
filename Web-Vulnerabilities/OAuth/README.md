...
But what is the reason for less security? Lets see how this flow goes.

- The client application requests user permission.
- After the user grants the permission, the OAuth provider redirects the user to the `redirect_uri` which contains the access token and related parameters in the URL parameters.
- After that the client application extracts the access token and gets the user info, from the OAuth the client application extracts the access token and gets the user info, from the OAuth provider. 

### Identification

So how do we identify which flow does the target application use?

As mentioned in the Authorization Code flow explaination, in **Step 3** the client applicaton includes a `grant_type` parameter to the request. In case of a authorization code, the value is `code` as observed earlier, andi n case of **Implicit**, the value is `token` (it's really straight forward).
